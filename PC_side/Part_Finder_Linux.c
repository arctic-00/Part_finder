...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (strcmp(name, "edit") == 0 || strcmp(name, "e") == 0) {
      edit_database(database_path);
    } else if (strcmp(name, "print_-a") == 0 || strcmp(name, "pa") == 0) {
      print_sorted(C_1_NUM_ROWS * C_1_NUM_COLS * 2, &components[0][0][0]);
    } else if (strcmp(name, "print") == 0 || strcmp(name, "p") == 0) {
...
void print_sorted(int n, char components[n][MAX_COMP_LEN + 1]) {
  ...
  // Copy across component array to a douple pointer so that it can be
