...
void pointLaser(byte buf[3]) {
  ...
  if (!(pos[0] >= 1 && pos[0] <= NUM_ROWS && pos[1] >= 1 && pos[1] <= NUM_COLS)) {
    error();
    return;
  }
...
int getAngle(byte axis, double initial_angle, double final_angle) {
  ...
  double tan_theta =
      tan(radians(initial_angle)) * (1 - p / num_segments) + tan(radians(final_angle)) * (p / num_segments);
